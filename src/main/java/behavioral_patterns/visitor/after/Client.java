...
public class Client {
     ...
     *
     * before 패키지에 Client 코드를 보면 rectangle 인스턴스는 인터페이스 타입으로 Java 언어에서는 해당 인스턴스에 printTo 메소드를 호출할때
     ...
     *
     * 즉 Shape 인터페이스에 printTo 메소드를 호출하려면 Shape 인터페이스에 구현클래스, Device 인터페이스에 구현클래스가 무었인지 2번 디스패치해야 추론이 가능하다.
     *
     * Shape 디스패치 1번
     * Device 디스패치 1번
     *
     ...
     *
     * after 패키지에 Client 코드를 보면 rectangle 인스턴스는 인터페이스 타입으로 Java 언어에서는 해당 인스턴스에 accept 메소드를 호출할때
     ...
     *
     * 즉 after 패키지에 print 기능은 1번에 디스패치 과정이 accept 메소드로 책임이 넘어갔기때문에 더블디스패치가 아니므로 컴파일 에러가 나지않고 동작한다.
